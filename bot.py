...
async def handle_session(message: types.Message):
    ...
    try:
        ...
        await message.answer(f"✅ Успешно авторизовались как {me.first_name}", 
                           reply_markup=main_menu())
...
async def show_chats(callback: types.CallbackQuery):
    ...
    async for dialog in current_client.get_dialogs(limit=50):
        chat = dialog.chat
        chat_id = chat.id
        title = chat.title or chat.first_name or chat.username or "Личный чат"
        title = title[:40]
    ...
    await callback.message.edit_text(
        f"📋 **Выберите чаты**\nВыбрано: {len(selected_chats)} чатов",
...
async def select_chat(callback: types.CallbackQuery):
    ...
    await show_chats(callback)  # обновляем клавиатуру
...
async def start_spam(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"✍️ Введите текст, который нужно разослать в **{len(selected_chats)}** чатов:"
    )
...
async def process_text(message: types.Message, state: FSMContext):
    ...
    await message.answer("⏱ Введите интервал между сообщениями в секундах (рекомендую 8–15):")
...
async def spam_task(message: types.Message, text: str, interval: int):
    ...
    for i, chat_id in enumerate(selected_chats[:], 1):
        try:
            ...
            await message.answer(f"❌ Чат {chat_id} недоступен")
        except Exception as e:
            await message.answer(f"❌ Ошибка в {chat_id}: {str(e)[:120]}")
...
async def stop_bot(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("🛑 Остановлено\n\nИспользуйте меню ниже", reply_markup=main_menu())
